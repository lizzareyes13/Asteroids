...
function controls(controlObj) {
  ...
  function keyDown(playerKeyPress) {
    ...
    if (keyPressed == 38) controlObj.upPressed = 1;
    if (keyPressed == 40) controlObj.downPressed = 1;
    if (keyPressed == 37) controlObj.leftPressed = 1;
    if (keyPressed == 39) controlObj.rightPressed = 1;
  }

  function keyUp(playerKeyPress) {
    var keyPressed = playerKeyPress.which;
    if (keyPressed == 38) controlObj.upPressed = 0;
    if (keyPressed == 40) controlObj.downPressed = 0;
    if (keyPressed == 37) controlObj.leftPressed = 0;
    if (keyPressed == 39) controlObj.rightPressed = 0;
  }
...
exports.default = generateAsteroids;
...
function gameLoop() {
  ...
  if (ctrl.upPressed == 1) ySpeed = Math.max(ySpeed - 1, -1 * maxSpeed);
  if (ctrl.downPressed == 1) ySpeed = Math.min(ySpeed + 1, 1 * maxSpeed);
  if (ctrl.rightPressed == 1) xSpeed = Math.min(xSpeed + 1, 1 * maxSpeed);
  if (ctrl.leftPressed == 1) xSpeed = Math.max(xSpeed - 1, -1 * maxSpeed);

  // deceleration
  if (ctrl.upPressed == 0 && ctrl.downPressed == 0) slowDownY();
  if (ctrl.leftPressed == 0 && ctrl.rightPressed == 0) slowDownX();
